...
public class NioChannel_bad {
    ...
    public NioChannel_bad(Boolean isServer,int socketPort, String serverIp) throws JAXBException, IOException  
    {
		...
		try
		{
			...
			System.out.println("Connected to server:" + serverIp + " port:" + socketPort);
			
			System.out.println("Starting msgRx...");
			msgTx();
			System.out.println("Starting msgTx...");
			msgRx();
		}
		catch(Exception ex)
		{
        ...
        new Thread("client-connection-manager") 
        {
            public void run() 
            {
                try 
                {
                    ...
                    System.out.println("FinishedConnect to " + PORT);
                    
                    while(!ssc.isConnected())
                    {
                    	System.out.println("Waiting on connection...");
                    }
                    ssc.configureBlocking(false);
                	isConnected = true;
                    System.out.println("Is Connecting to " + PORT);
        ...
        new Thread("Message-receive") 
        {
            public void run() 
            {
                try 
                {
                    ...
                    while (PluginEngine.nioChannelIsActive) 
                    {
                    	System.out.println("PreSelector");
                    	selector.select();
                    	System.out.println("PostSelector");
                        ...
                        while (it.hasNext()) 
                        {
                         ...
                         //System.out.println("Hashcode: " + hashcode);   	
                            if ((key.readyOps() & SelectionKey.OP_ACCEPT) == SelectionKey.OP_ACCEPT) 
                            {
                               ...
                               //for(String str : sstr)
                               //if((msg.startsWith("<\\?xml version=\"1\\.0\" encoding=\"UTF-8\" standalone=\"yes\"\\?>\n")) &&  msg.endsWith("</ns2:LogEvent>\n"))
                               //if((msg.startsWith("<\\?xml")) &&  msg.endsWith("</ns2:LogEvent>\n"))
                                //System.out.println("*" + msg.substring(0, 10) + "*");
                                //System.out.println("*" + Integer.toHexString( msg.charAt(0) ) + "*" +  Integer.toHexString( msg.charAt(1) ));
                                /*
                                ...
                                if((msg.startsWith("<?xml")) && (msg.endsWith("</ns2:LogEvent>\n")))
                                {
                                    ...
                                    for(int i = 1; i <sstr.length; i++)
                                    {
                                    	String inputMsg = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n" + sstr[i];
                                   	 	//getMsgEvents(inputMsg,hashcode);
                                       MsgEvent me = getMsgEvent(inputMsg);
                             		   if(me != null)
                             		   {
                             			   if((me.getParam("src_region") != null))
                             			   {
                             				   ...
                             				   {
                             					   ahm.put(msgHash, hashcode);
                             					   System.out.println("Added: *" + msgHash + "* code:" + hashcode + " size:" + ahm.size());
                             				   }
                             				   else
                             				   {
                             		   ...
                             		   {
                             			   System.out.println("hashcode: " + hashcode + " input:" + inputMsg);
                            	 ...
                            	 //System.out.println(msg);
                            	   ...
                            	   if(!str.endsWith("</ns2:LogEvent>\n"))
                            		   ...
                            		   */
                            			   //String inputMsg = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n" + str;
                                	//System.out.println("*" + Integer.toHexString( msg.charAt(msg.length()-2) ) + "*" +  Integer.toHexString( msg.charAt(msg.length()-1) ));
                                	//System.out.println("*" + msg.charAt(0) + "*" +  msg.charAt(1) + "..." + msg.charAt(msg.length()-2) + "*" + msg.charAt(msg.length()-1));
    ...
    private static void getMsgEvents(String inputMsg, int hashcode)
    {
    	MsgEvent me = getMsgEvent(inputMsg);
		   if(me != null)
		   {
                    			...
                    			if((me.getParam("dst_region") != null))
                    			{
                    				...
                    				if(ahm.containsKey(msgHash))
                    				{
                    					...
                    					if(chm.containsKey(socketHash))
                    					{
                    						...
                    						{
                    							System.out.println("msgTx: " + ex.toString());
                    							sc.close();
